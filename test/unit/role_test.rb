...
class RoleTest < ActiveSupport::TestCase
  test "valid role" do
    valid_role = FactoryGirl.build(:role)
    assert valid_role.valid?
  end

  test "invalid role" do
    invalid_role = FactoryGirl.build(:role, :role => "invalidRole")
  ...
  test "uniqueness of role" do
    valid_role = FactoryGirl.create(:role)
    duplicate_role = Role.new(:playlist_id => valid_role.playlist_id, :user_id => valid_role.user_id, :role => "Creator")
